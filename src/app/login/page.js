'use client'

import { useState } from 'react'
import Link from 'next/link'
import { login, signup, requestPasswordReset, signInWithGoogle } from './actions'
import BrandMark from '@/components/BrandMark'
import styles from './login.module.css'

const MODES = {
  login: { title: 'Bon retour', submit: 'Se connecter' },
  signup: { title: 'Créer un compte', submit: 'Créer mon compte' },
  reset: { title: 'Mot de passe oublié', submit: 'Envoyer le lien' },
}

export default function LoginPage() {
  const [mode, setMode] = useState('login')
  const [showPassword, setShowPassword] = useState(false)
  const [message, setMessage] = useState(null) // { type: 'error' | 'info', text }
  const [isLoading, setIsLoading] = useState(false)

  const switchMode = (next) => {
    setMode(next)
    setMessage(null)
  }

  const handleSubmit = async (e) => {
    e.preventDefault()
    setIsLoading(true)
    setMessage(null)
    const formData = new FormData(e.target)

    try {
      const action = mode === 'login' ? login : mode === 'signup' ? signup : requestPasswordReset
      const result = await action(formData)

      if (result?.error) {
        setMessage({ type: 'error', text: result.error })
      } else if (result?.info) {
        setMessage({ type: 'info', text: result.info })
      } else if (result?.redirectTo) {
        window.location.assign(result.redirectTo)
        return
      }
    } catch (err) {
      setMessage({ type: 'error', text: "Une erreur est survenue. Réessayez." })
    } finally {
      setIsLoading(false)
    }
  }

  return (
    <div className={styles.page}>
      <aside className={styles.visual}>
        <img src="/promo-pizza.png" alt="" className={styles.photo} />
        <div className={styles.visualContent}>
          <BrandMark size="lg" />
          <p className={styles.tagline}>Sortie du four,<br />livrée chez vous.</p>
        </div>
      </aside>

      <main className={styles.panel}>
        <div className={styles.card}>
          <h1 className={styles.title}>{MODES[mode].title}</h1>
          <p className={styles.intro}>
            {mode === 'login' && 'Connectez-vous pour commander et suivre vos livraisons.'}
            {mode === 'signup' && 'Quelques secondes suffisent pour commander.'}
            {mode === 'reset' && 'Entrez votre email : nous vous enverrons un lien pour choisir un nouveau mot de passe.'}
          </p>

          {message && (
            <p className={message.type === 'error' ? styles.error : styles.info} role="alert">{message.text}</p>
          )}

          <form onSubmit={handleSubmit} className={styles.form}>
            {mode === 'signup' && (
              <div className={styles.field}>
                <label htmlFor="first_name">Prénom</label>
                <input id="first_name" name="first_name" type="text" autoComplete="given-name" placeholder="Russel" required />
              </div>
            )}

            <div className={styles.field}>
              <label htmlFor="email">Email</label>
              <input id="email" name="email" type="email" autoComplete="email" required placeholder="vous@exemple.com" />
            </div>

            {mode !== 'reset' && (
              <div className={styles.field}>
                <div className={styles.labelRow}>
                  <label htmlFor="password">Mot de passe</label>
                  {mode === 'login' && (
                    <button type="button" className={styles.linkBtn} onClick={() => switchMode('reset')}>
                      Mot de passe oublié ?
                    </button>
                  )}
                </div>
                <div className={styles.passwordWrap}>
                  <input
                    id="password"
                    name="password"
                    type={showPassword ? 'text' : 'password'}
                    autoComplete={mode === 'login' ? 'current-password' : 'new-password'}
                    required
                    minLength={mode === 'signup' ? 8 : undefined}
                    placeholder={mode === 'signup' ? '8 caractères minimum' : '••••••••'}
                  />
                  <button
                    type="button"
                    className={styles.eye}
                    onClick={() => setShowPassword(v => !v)}
                    aria-label={showPassword ? 'Masquer le mot de passe' : 'Afficher le mot de passe'}
                  >
                    {showPassword ? (
                      <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24" /><line x1="1" y1="1" x2="23" y2="23" /></svg>
                    ) : (
                      <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>
                    )}
                  </button>
                </div>
              </div>
            )}

            <button type="submit" className={styles.submit} disabled={isLoading}>
              {isLoading ? 'Un instant…' : MODES[mode].submit}
            </button>
          </form>

          {mode !== 'reset' && (
            <>
              <div className={styles.divider}><span>ou</span></div>

              <form action={signInWithGoogle}>
                <button type="submit" className={styles.google}>
                  <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
                  </svg>
                  Continuer avec Google
                </button>
              </form>
            </>
          )}

          <p className={styles.switch}>
            {mode === 'login' && (
              <>Pas encore de compte ? <button type="button" onClick={() => switchMode('signup')}>Créer un compte</button></>
            )}
            {mode === 'signup' && (
              <>Vous avez déjà un compte ? <button type="button" onClick={() => switchMode('login')}>Se connecter</button></>
            )}
            {mode === 'reset' && (
              <button type="button" onClick={() => switchMode('login')}>Revenir à la connexion</button>
            )}
          </p>

          <Link href="/" className={styles.guest}>Continuer sans compte</Link>
        </div>
      </main>
    </div>
  )
}
